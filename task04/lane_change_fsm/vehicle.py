...
class Vehicle(object):
        ...
        if vehicle_ahead:
            ...
            if vehicle_behind:
                ...
                max_velocity_in_front = (
                                                    vehicle_ahead.s - self.s - self.preferred_buffer) + vehicle_ahead.v - 0.5 * self.a
    ...
    def prep_lane_change_trajectory(self, predictions, state):
        ...
        if self.get_vehicle_behind(predictions, self.lane):
            ...
            s, v, a = min([next_lane_new_kinematics, curr_lane_new_kinematics],
                          key=lambda x: x[1])  # choose kinematics with lowest velocity
